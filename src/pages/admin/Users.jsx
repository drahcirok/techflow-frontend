...
const Users = () => {
  ...
  const fetchUsers = async () => {
    setIsLoading(true);
    try {
      // Intenta obtener usuarios del backend
      ...
      if (error.response?.status === 403) {
        ...
        toast.error('Endpoint de usuarios no encontrado');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(`${formData.role === 'TECNICO' ? 'Técnico' : 'Usuario'} creado exitosamente`);
      setShowModal(false);
      setFormData({
        name: '',
        email: '',
  ...
  const getRoleBadge = (role) => {
    const styles = {
      ...
      ROLE_ADMIN: 'bg-sky-100 text-sky-700',
      ROLE_TECNICO: 'bg-sky-100 text-sky-700',
      ROLE_CLIENTE: 'bg-violet-100 text-violet-700',
    };
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-500">Administra técnicos y usuarios del sistema</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-slate-800">{users.filter(u => u.role === 'ADMIN' || u.role === 'ROLE_ADMIN').length}</p>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-slate-800">{users.filter(u => u.role === 'TECNICO' || u.role === 'ROLE_TECNICO').length}</p>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-slate-800">{users.filter(u => u.role === 'CLIENTE' || u.role === 'ROLE_CLIENTE').length}</p>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-200">
          <h2 className="text-lg font-semibold text-slate-800">Lista de Usuarios</h2>
        </div>
                  ...
                  <tr key={user.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                      ...
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                          user.role === 'ADMIN' || user.role === 'ROLE_ADMIN' 
                            ? 'bg-sky-100' 
                            : user.role === 'TECNICO' || user.role === 'ROLE_TECNICO'
                            ? 'bg-sky-100'
                            : 'bg-violet-100'
                        }`}>
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className={`px-3 py-1 rounded-lg text-xs font-medium ${
                        ...
                        {user.status === 'active' || user.active !== false ? 'Activo' : 'Inactivo'}
                      </span>
                    </td>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
            <div className="flex items-center justify-between p-6 border-b border-slate-200">
              <div>
                ...
                <p className="text-sm text-slate-500">Crear técnico o administrador</p>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent"
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    onChange={handleInputChange}
                    className="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent pr-12"
                    placeholder="••••••••"
                  />
                  <button
                    type="button"
